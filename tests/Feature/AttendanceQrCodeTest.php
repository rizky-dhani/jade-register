...
test('user can view QR code page with valid token', function () {
    livewire(\App\Livewire\AttendanceQrCode::class, ['token' => $this->registration->qr_token])
...
test('user sees expired message with expired token', function () {
    ...
    livewire(\App\Livewire\AttendanceQrCode::class, ['token' => $expiredRegistration->qr_token])
...
test('user sees invalid message with invalid token', function () {
    livewire(\App\Livewire\AttendanceQrCode::class, ['token' => 'invalid-token-123'])
        ->assertSee('Invalid QR Code')
        ->assertDontSee('registration_code');
});

test('QR code page shows hands-on sessions', function () {
    $handsOn = \App\Models\HandsOn::factory()->create();
    $handsOnReg = \App\Models\HandsOnRegistration::factory()->verified()->create([
...
test('QR code page does not show unverified hands-on sessions', function () {
    $handsOn = \App\Models\HandsOn::factory()->create();
    $handsOnReg = \App\Models\HandsOnRegistration::factory()->create([
        'seminar_registration_id' => $this->registration->id,
        'hands_on_id' => $handsOn->id,
...
test('QR code page shows payment status correctly', function () {
    ...
    livewire(\App\Livewire\AttendanceQrCode::class, ['token' => $pendingRegistration->qr_token])
        ->assertSee('Pending');

    $rejectedRegistration = SeminarRegistration::factory()->rejected()->create();

    livewire(\App\Livewire\AttendanceQrCode::class, ['token' => $rejectedRegistration->qr_token])
        ->assertSee('Rejected');
});
