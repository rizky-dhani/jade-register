<div class="min-h-screen bg-gray-50 py-8 px-4">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
            ...
            <h1 class="text-2xl font-bold text-green-800 mb-2">{{ __('seminar.hands_on_success_title') }}</h1>
            <p class="text-green-700 mb-4">{{ __('seminar.hands_on_success_thank_you') }}{{ $registration->name ?? $registration->name_license }}!</p>
            ...
            @if(count($handsOnSessions))
                <div class="mt-6 pt-6 border-t border-green-200">
                    <h2 class="text-lg font-semibold text-green-800 mb-4">{{ __('seminar.hands_on_sessions_title') }}</h2>
                    <div class="space-y-3 text-left">
                        @foreach($handsOnSessions as $session)
                            <div class="bg-white border border-green-200 rounded-lg p-4 flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    @if($session['code'])
                                        <span class="inline-flex items-center px-1.5 py-0.5 text-xs font-semibold text-gray-600 bg-gray-200 rounded">
                                            {{ $session['code'] }}
                                        </span>
                                    @endif
                                    <p class="text-sm font-medium text-gray-800 mt-1">{{ $session['name'] }}</p>
                                    @if($session['doctor'])
                                        <p class="text-xs text-gray-500">{{ $session['doctor'] }}</p>
                                    @endif
                                    @if($session['date'])
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $session['date'] }}</p>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                     ...
                     x-init="
                        const timer = setInterval(() => {
                            if (countdown > 0) {
                                ...
                                clearInterval(timer);
                                window.location.href = '{{ route('register.hands-on', ['locale' => app()->getLocale()]) }}';
                            }
                        }, 1000);
                     ">
